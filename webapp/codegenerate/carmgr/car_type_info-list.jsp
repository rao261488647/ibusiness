...
<html>
  ...
  <head>
    ...
    <script type="text/javascript">
		var config = {
			...
			gridFormId: 'gridForm',
	        exportUrl: 'car_type_info-export.do'
		};
  ...
  <body>
	...
	<!-- 查询条件 -->
        <div class="panel-heading"><h4 class="panel-title glyphicon glyphicon-paperclip">查询</h4></div>
          <div class="panel-body">
	          <div id="search" class="content content-inner">
				  <form name="cgForm" method="post" action="car_type_info-list.do" class="form-inline">
				    <div class="form-group">
				                <label for="code_table_typename">车型名称:</label>
	   ...
	   <div class="panel-heading"><h4 class="panel-title glyphicon glyphicon-paperclip">车型管理列表</h4></div>
       <div class="panel-body">
		    <div class="pull-left">
			    <button class="btn btn-primary btn-sm a-insert" href="car_type_info-input.do" data-target="#modalInput" data-toggle="modal" data-database="true"><span class="glyphicon glyphicon-tasks"></span>新建</button>
	   ...
	   <div class="content">
			<form id="gridForm" name="gridForm" method='post' action="car_type_info-remove.do" class="m-form-blank">
			  <table id="codeGrid" class="table table-hover table-striped">
			      <thead>
				      <tr>
				        <th width="30" class="m-table-check"><input type="checkbox" name="checkAll" onchange="toggleSelectedItems(this.checked)"></th>
					                <th class="sorting">车型名称</th>
					                <th class="sorting">动力类型</th>
					                <th class="sorting">车辆类型</th>
					                <th class="sorting">车辆箱数</th>
					                <th class="sorting">变速箱</th>
					                <th class="sorting">座位数</th>
					                <th class="sorting">排量</th>
					                <th class="sorting">发动机功率</th>
					                <th class="sorting">上市年份</th>
				        <th width="30">&nbsp;</th>
				      </tr>
				    </thead>
					    <tbody>
					      ...
					      <tr class="${status.index%2==1? 'active':''}">
					        <td><input type="checkbox" class="selectedItem a-check" name="selectedItem" value="${item.id}"></td>
						            ...
						            <td>${item.swept}</td>
						            <td>${item.displayment}</td>
						            <td>${item.listyear}</td>
					        <td>
					          <a href="car_type_info-input.do?id=${item.id}" class="a-update" data-target="#modalInput" data-toggle="modal" data-database="true"><span class="glyphicon glyphicon-pencil"></span></a>
					        </td>
					      </tr>
					      </c:forEach>
					    </tbody>
