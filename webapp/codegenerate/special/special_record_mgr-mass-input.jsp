    ...
    <div class="panel panel-default col-lg-12 col-md-12 col-sm-12"> 
        ...
        <div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-lg-3 col-md-3 col-sm-3">
						<input id="code-drivername" type="text" name="drivername" value="${model.drivername}" class="text form-control input-sm " readonly>
						...
						<script type="text/javascript">
						    ...
						    function selectedItemCell() {
						    	...
						    	var customercell = "";
						    	$('.selectedItem:checked').each(function(){ //由于复选框一般选中的是多个,所以可以循环输出
						    	   customercell = customercell + "," + $(this).val();
						    	});
								...
								$.ajax({
										...
										success : function(jsonData) {
											...
											$.each(
												...
												function(i, item) {
													var newRow = '<tr><td>'
															+'<input type="checkbox" class="selectedItem a-check" name="selectedItem" value="'+item.customercell+'">' 
															+'</td><td>'
															+ item.customername + '</td><td>' + item.customercell
															+ '</td></tr>';
													$('#drivernameRowadd').append(newRow);
												});
									    },
						...
						<div id="drivernameSInputDiv" class="modal fade" tabindex="-1" style="top: 20%;">
							<div class="modal-dialog">
								<div class="modal-content">
									<div class="modal-header">
										<a href="#" class="close btn btn-primary btn-sm" onclick="$('#drivernameSInputDiv').modal('hide');"><span>&times;</span><span class="sr-only">Close</span></a>
										...
										<div class="form-group">
											...
											<a href="#" class="btn btn-primary btn-sm" onClick="selectedItemCell('${item.customername}','${item.customercell}')">选择</a>
										</div>
									</div>
									<div class="modal-body">
										<div class="content">
											<table id="codeGrid" class="table table-hover table-bordered">
												...
												<tbody id='drivernameRowadd'>
													<c:forEach items="${drivernamePage.result}" var="item">
														<tr>
														    <td><input type="checkbox" class="selectedItem a-check" name="selectedItem" value="${item.customercell}"></td>
									...
									<div class="modal-footer">
										<a href="#" class="btn btn-primary btn-sm"
											onclick="$('#drivernameSInputDiv').modal('hide');">关闭</a>
									</div>
								</div>
							</div>
						</div>
