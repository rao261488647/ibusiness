package com.xg.src.com.tencent.xinge;
...
import com.xg.outerlib.org.json.JSONArray;
...
public class XingeApp {
	...
	// 易用的api接口v1.1.4引入
	public static JSONObject pushTokenAndroid(long accessId, String secretKey,
	...
	public static JSONObject pushAccountAndroid(long accessId,
			String secretKey, String title, String content, String account) {
		Message message = new Message();
	...
	public static JSONObject pushTagAndroid(long accessId, String secretKey,
	...
	public static JSONObject pushTokenIos(long accessId, String secretKey,
			String content, String token, int env) {
	...
	public static JSONObject pushAccountIos(long accessId, String secretKey,
			String content, String account, int env) {
		MessageIOS message = new MessageIOS();
		message.setAlert(content);
		message.setBadge(1);
		message.setSound("beep.wav");
	...
	public static JSONObject pushAllIos(long accessId, String secretKey,
			String content, int env) {
		MessageIOS message = new MessageIOS();
		message.setAlert(content);
		message.setBadge(1);
		message.setSound("beep.wav");
	...
	public static JSONObject pushTagIos(long accessId, String secretKey,
			String content, String tag, int env) {
		MessageIOS message = new MessageIOS();
		message.setAlert(content);
		message.setBadge(1);
		message.setSound("beep.wav");
	...
	public JSONObject pushSingleDevice(String deviceToken, Message message) {
		if (!ValidateMessageType(message)) {
			return new JSONObject(
					"{'ret_code':-1,'err_msg':'message type error!'}");
		}
		if (!message.isValid()) {
			return new JSONObject(
	...
	public JSONObject pushSingleDevice(String deviceToken, MessageIOS message,
		...
		if (!ValidateMessageType(message, environment)) {
			return new JSONObject(
					"{'ret_code':-1,'err_msg':'message type or environment error!'}");
		}
		if (!message.isValid()) {
			return new JSONObject(
					"{'ret_code':-1,'err_msg':'message invalid!'}");
	...
	public JSONObject pushSingleAccount(int deviceType, String account,
			MessageIOS message, int environment) {
		if (!ValidateMessageType(message, environment)) {
			return new JSONObject(
					"{'ret_code':-1,'err_msg':'message type or environment error!'}");
		}
	...
	public JSONObject pushAccountListMultiple(int pushId,
	...
	protected boolean ValidateMessageType(MessageIOS message, int environment) {
		if (this.m_accessId >= IOS_MIN_ID
				&& (environment == IOSENV_PROD || environment == IOSENV_DEV))
			return true;
		else
			return false;
	}
