...
public class CodeTagFactory {
    ...
    static {
        ...
        // 富文本编辑器组件
        tagComponentMap.put(Constants.CODE_KINDEDITOR, "kindeditorParser");
    }
	...
	public Columnt checkBoxParser(Columnt columnt, ConfFormTableColumn formColumn) {
    	...
    	JSONArray jsonArray= JSONArray.fromObject(confSelectInfo);
        // 循环遍历
        String str = "";
        str = str + "<div class=\"control-label col-lg-3 col-md-3 col-sm-3\">";
        Iterator<Object> jsonit = jsonArray.iterator();
        while (jsonit.hasNext()) {
        	JSONObject jo = (JSONObject) jsonit.next();
        	str = str + "<input type=\"checkbox\" name=\""+columnt.getFieldName()+"\" value=\""+jo.getString("key")+"\"  <c:if test=\"${fn:contains(model."+columnt.getFieldName()+", '"+jo.getString("key")+"')}\">checked</c:if>>";
        	str = str + "<label style=\"display:inline;\">"+jo.getString("value")+"</label>&nbsp";
    	}
        str = str + "</div>";
        columnt.setJspTagInfo(str);
        return columnt;
	...
	public Columnt selectInputParser(Columnt columnt, ConfFormTableColumn formColumn) {
        ...
        controllerInfo = controllerInfo + "public String ajax"+jsonObject.getString("queryName")+"list(@ModelAttribute Page page, @RequestParam Map<String, Object> parameterMap) {";
        // 查询条件Filter过滤器
        ...
        str = str + "   	function change"+jsonObject.getString("queryName")+"Value(";
        ...
        str = str + "       }";
        // ajax call 调用后台进行查询
        str = str + "function searchOwnername"+jsonObject.getString("queryName")+"(urlStr) { ";
        str = str + " $.ajax({ ";
        str = str + " 	 type: \"POST\", ";
        str = str + "	 url: \"/\"+window.location.pathname.split(\"/\")[1]+\"/\"+urlStr, ";
        ...
        str = str + "		   var newRow=' ";
        str = str + "           	     <tr>";
        str = str + "           	        <td><a href=\"#\" class=\"btn btn-primary btn-sm\" onClick=\"change"+jsonObject.getString("queryName")+"Value(\\''+";
		for (int i=0; i<list.size(); i++) {
			if (0 != i) {
				str = str + "+'\\',\\''+";
			}
        ...
        str = str + "             <a class=\"btn btn-primary btn-sm\" href=\"#\" onclick=\"searchOwnername"+jsonObject.getString("queryName")+"('"+CommonUtils.toLowerCase(formColumn.getTableName()).replace("ib_", "")+"/"+jsonObject.getString("queryName")+"-list.do')\"><span class=\"glyphicon glyphicon-search\"></span>查询</a> ";
        str = str + "           </div> ";
        str = str + "         </div>";
     ...
     *  富文本编辑器组件
     */
    public Columnt kindeditorParser(Columnt columnt, ConfFormTableColumn formColumn) {
        String jspFormStr = "<!-- KindEditor文本编辑器 --> ";
        jspFormStr = jspFormStr + "<!-- 富文本编辑器 --> ";
        jspFormStr = jspFormStr + "<script charset=\"utf-8\" src=\"${ctx}/plugin/kindeditor/kindeditor.js\"></script> ";
        jspFormStr = jspFormStr + "<script charset=\"utf-8\" src=\"${ctx}/plugin/kindeditor/lang/zh_CN.js\"></script> ";
        jspFormStr = jspFormStr + "<script charset=\"utf-8\" src=\"${ctx}/plugin/kindeditor/plugins/code/prettify.js\"></script> ";
        jspFormStr = jspFormStr + "<script type=\"text/javascript\"> ";
        jspFormStr = jspFormStr + "$(function() { ";
        jspFormStr = jspFormStr + "    var options = { ";
        jspFormStr = jspFormStr + "        cssPath : '${ctx}/plugin/kindeditor/plugins/code/prettify.css', ";
        jspFormStr = jspFormStr + "        filterMode : true, ";
        jspFormStr = jspFormStr + "        uploadJson:'${ctx}/common/upload.do', ";
        jspFormStr = jspFormStr + "        width : '860px', ";
        jspFormStr = jspFormStr + "        height:'400px', ";
        jspFormStr = jspFormStr + "        resizeType : 1, ";
        jspFormStr = jspFormStr + "        allowFlashUpload : false, ";
        jspFormStr = jspFormStr + "        allowMediaUpload : false, ";
        jspFormStr = jspFormStr + "        allowFileManager : false, ";
        jspFormStr = jspFormStr + "        syncType:\"form\", ";
        jspFormStr = jspFormStr + "        afterCreate : function() {var self = this; self.sync();}, ";
        jspFormStr = jspFormStr + "        afterChange : function() {var self = this; self.sync();}, ";
        jspFormStr = jspFormStr + "        afterBlur : function() {var self = this; self.sync();}, ";
        jspFormStr = jspFormStr + "        items:['source', '|', 'fullscreen', 'undo', 'redo', 'print', 'cut', 'copy', 'paste', 'plainpaste', 'wordpaste', '|', 'justifyleft', 'justifycenter', 'justifyright', 'justifyfull', 'insertorderedlist', 'insertunorderedlist', 'indent', 'outdent', 'subscript', 'superscript', '|', 'selectall', 'clearhtml','quickformat','|', 'formatblock', 'fontname', 'fontsize', '|', 'forecolor', 'hilitecolor', 'bold', ";
        jspFormStr = jspFormStr + "            'italic', 'underline', 'strikethrough', 'lineheight', 'removeformat', '|', 'image','flash', 'media', 'table', 'hr', 'emoticons', 'link', 'unlink', '|', 'about'] ";
        jspFormStr = jspFormStr + "    }; ";
        jspFormStr = jspFormStr + "    KindEditor.create('#code-"+columnt.getFieldName()+"',options); ";
        jspFormStr = jspFormStr + "}); ";
        jspFormStr = jspFormStr + "</script> ";
        //
        List<String> jspFormList = columnt.getJspFormList();
        ...
        // 取得参数里面最大 / 最小 输入长度
        String str = "";
        str = str + "<div class=\"col-lg-10 col-md-10 col-sm-10\">";
        str = str + "    <textarea class=\"form-control "+("1".equals(formColumn.getFcMust())? "required" : "")+"\" id=\"code-"+columnt.getFieldName()+"\" name=\""+columnt.getFieldName()+"\" style=\"width:100%;height:400px;visibility:hidden;\" maxlength=\"50\" >${model."+columnt.getFieldName()+"}</textarea>";
        str = str + "</div>";
        columnt.setJspTagInfo(str);
        // java相关信息
        return columnt;
